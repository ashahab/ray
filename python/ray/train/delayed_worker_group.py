...
from ray.train.worker_group import WorkerGroup, BaseWorkerMixin, Worker, WorkerMetadata,\
    create_executable_class, construct_metadata
T = TypeVar("T")
...
class DelayedWorkerGroup(WorkerGroup):
    ...
    def add_worker(self, hostname: str, rank: int):
        """Adds ``num_workers`` to this WorkerGroup.

        Args:
            num_workers (int): The number of workers to add.
        """
        additional_resources = self.additional_resources_per_worker.copy()
        additional_resources[f"node:{hostname}"] = 0.01
        remote_cls = ray.remote(
            num_cpus=self.num_cpus_per_worker,
            num_gpus=self.num_gpus_per_worker,
            resources=additional_resources)(self._base_cls)



        actor = remote_cls.remote(*self._actor_cls_args,
                                    **self._actor_cls_kwargs)

        # Get metadata from all actors.
        metadata = ray.get(actor._BaseWorkerMixin__execute.remote(construct_metadata))
        worker = Worker(actor=actor, metadata=metadata)
